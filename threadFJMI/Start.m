...
time2 = []; %The time final averaged time for FJMI %***this variable will be updated in the second run
selected2 = []; %The features selected for FJMI  %***this variable will be updated in the second run
...
parpool(48);
poolobj = gcp;
addAttachedFiles(poolobj,{'FJMI.m', 'joint.m'});

parfor i=1:boot
    bo = bootstrap(i,:); 
    [selected_FJMI,time_FJMI] = FJMI(index, k, threshold, bo);    
     
    
    selected2(i,:) = selected_FJMI;
    time2(i,:) = time_FJMI;
end
